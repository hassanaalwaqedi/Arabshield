...
import {
    Briefcase, MapPin, Building2, ArrowRight,
    CheckCircle, Upload, X, Loader2, Send, FileText, Copy, Check
} from 'lucide-react';
import { getJobById, submitApplication, Job } from '@/lib/careersService';
        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center text-foreground">
                <div className="text-center">
                    ...
                    <h1 className="text-2xl font-bold mb-2">{t('jobNotFound')}</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="bg-card/50 border border-border rounded-2xl p-6">
                            <h2 className="text-xl font-bold mb-4">{t('aboutJob')}</h2>
                        ...
                        {job.responsibilities && job.responsibilities.length > 0 && (
                            <div className="bg-card/50 border border-border rounded-2xl p-6">
                                <h2 className="text-xl font-bold mb-4">{t('responsibilities')}</h2>
                                <ul className="space-y-3">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-card/50 border border-border rounded-2xl p-6 sticky top-24">
                            <h3 className="text-lg font-bold mb-4">{t('applyNow')}</h3>
                            <p className="text-muted-foreground text-sm mb-6">
                                {job.status === 'open' ? t('jobOpen') : t('jobClosed')}
                            </p>
                            <button
                            ...
                            <div className="mt-6 pt-6 border-t border-border">
                                <h4 className="text-sm font-medium text-muted-foreground mb-3">{t('shareJob')}</h4>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-card border border-border rounded-3xl p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-6">
                    <h3 className="text-2xl font-bold text-foreground">{t('applicationTitle')}</h3>
                ...
                <p className="text-muted-foreground mb-6">
                    {t('applyingFor')} <span className="text-foreground font-medium">{job.title}</span>
                </p>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-2">
                            {t('cv')} <span className="text-blue-400">*</span>
                        </label>
                            ...
                            >
                                {cvFile ? (
                                    ...
                                    <>
                                        ...
                                        <span className="text-muted-foreground">{t('cvUpload')}</span>
                                    </>
                                )}
                            </label>
                        </div>
                        <p className="mt-2 text-xs text-muted-foreground">{t('cvFormat')}</p>
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-2">
                            {t('coverLetter')} <span className="text-blue-400">*</span>
                        </label>
                        <textarea
