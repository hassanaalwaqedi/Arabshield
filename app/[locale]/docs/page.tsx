        ...
        <div className="min-h-screen bg-background text-foreground">
            ...
            <div className="relative overflow-hidden">
                ...
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-20 text-center">
                    ...
                    {searchQuery && filteredArticles.length > 0 && (
                        <div className="max-w-2xl mx-auto mt-4 bg-card border border-border rounded-2xl overflow-hidden text-right">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 pb-12">
                <div className="flex flex-wrap gap-4 justify-center">
                    <Link href="/tutorials" className="inline-flex items-center gap-2 px-6 py-3 bg-card border border-border rounded-xl hover:border-blue-500/50 transition-colors">
                        <Sparkles className="w-5 h-5 text-purple-400" />
                        <span>دروس الفيديو</span>
                    </Link>
                    <Link href="/issues" className="inline-flex items-center gap-2 px-6 py-3 bg-card border border-border rounded-xl hover:border-blue-500/50 transition-colors">
                        <FileText className="w-5 h-5 text-yellow-400" />
                        <span>المشاكل الشائعة</span>
                    </Link>
                    <a href="https://github.com/NovaArab" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 bg-card border border-border rounded-xl hover:border-blue-500/50 transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {docCategories.map((category) => {
                        ...
                        return (
                            <div
                                key={category.id}
                                className="group bg-card border border-border rounded-2xl p-6 hover:border-blue-500/50 transition-all hover:shadow-xl hover:shadow-blue-500/10"
                            >
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-muted-foreground">{category.articleCount} مقالة</p>
                                    </div>
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {docArticles.slice(0, 6).map((article) => (
                            <Link
                                key={article.id}
                                href={`/docs/${article.category}/${article.slug}`}
                                className="group bg-card/50 border border-border rounded-xl p-6 hover:border-blue-500/50 transition-all"
                            >
            ...
            <div className="border-t border-border">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
                    ...
                    <p className="text-muted-foreground mb-8">فريق الدعم لدينا جاهز لمساعدتك</p>
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <Link
                            href="/support"
                            className="px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-foreground rounded-xl font-semibold hover:shadow-lg hover:shadow-blue-600/30 transition-all"
                        >
                            تواصل مع الدعم
                        </Link>
                        <Link
